...
`mwsync.bash` provides Bash tab completion for `mwsync.py`. It completes
subcommands, common options, registered article keys, and configured local
`.mw` filenames from the current directory's `mwsync.yaml`. It only reads local
files and never calls the MediaWiki API.
