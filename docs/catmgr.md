...
Automatic discovery of parent categories from Wikidata or enwiki should be a
future explicit feature because it requires new network behavior. When added,
it should never run as a surprising side effect of local cache inspection.

Prefer Wikidata and other CC0 structured data for generated descriptive content.
This avoids importing Wikipedia category-page prose and the associated
CC BY-SA attribution requirements. If future modes read an enwiki category page,
they should treat that page primarily as a source of category relationships or
manual-review hints, not as text to copy.
